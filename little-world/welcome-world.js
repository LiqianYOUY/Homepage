...
export function createWelcomeWorld({getVisitor,getCommunity,refreshCommunity,postcard,getSmart,getTerrace,toast,onStateChange,onTV,onMusic,onBooks,onCat,onWave}){
 ...
 async function renderProfile(){const {content}=panel('profile','关于这间小屋','认识一点点，再一起玩一会儿。',650);const intro=node('div','host-intro'),avatar=node('div','host-portrait');avatar.innerHTML=hostAvatar;const copy=node('div');copy.append(node('span','dialog-eyebrow','YOUR HOST'),node('h3','','你好呀，我是小尤。'),node('p','','喜欢做原型、研究人与技术如何相处。关心负责任的 AI，也关心吃饭、吃药和情绪这些日常小事。'));intro.append(avatar,copy);content.append(intro);
