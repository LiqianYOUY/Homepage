...
import {createWelcomeWorld} from './welcome-world.js';
import {setupTelevision} from './television.js';
import {houseIcon,visitorAvatar} from './little-icons.js';
...
const communityClient=await createCommunityClient();visitor=communityClient.visitor;community=communityClient.community;
...
const keys=new Set();
function enterWalk(){if(!loadDone)return;watchingCat=false;walk=true;moveTween=null;controls.enabled=false;camera.position.copy(P(1130,542,1.57));lookYaw=0;lookPitch=0;camera.rotation.set(lookPitch,lookYaw,0,'YXZ');document.body.classList.add('walk-mode');$('#walk-mode').classList.add('active');$('#walk-mode').innerHTML='⌂ <span>退出行走</span>';$('#controls-help').textContent='WASD / 方向键移动 · 拖动转头 · 点击物品 · Esc 退出';toast('用 WASD 走动，拖动鼠标看向四周。');}
function exitWalk(restore=true){walk=false;keys.clear();controls.enabled=true;document.body.classList.remove('walk-mode');$('#walk-mode').classList.remove('active');$('#walk-mode').innerHTML='♧ <span>走进家里</span>';$('#controls-help').textContent='拖动旋转 · 滚轮缩放 · 点击物品使用 · 拖动门和椅子';if(restore)go('overview');}
...
window.addEventListener('keydown',e=>{if(e.defaultPrevented)return;if(typing(e))return;if(e.key==='Escape'){if(!$('#portfolio-window').hidden)closePortfolio();else if(walk)exitWalk();return;}if(walk&&['w','a','s','d','arrowup','arrowdown','arrowleft','arrowright','shift'].includes(e.key.toLowerCase())){e.preventDefault();keys.add(e.key.toLowerCase());}});
window.addEventListener('keyup',e=>keys.delete(e.key.toLowerCase()),true);document.addEventListener('focusin',e=>{if(typing(e))keys.clear();});window.addEventListener('blur',()=>keys.clear());document.addEventListener('visibilitychange',()=>{if(document.hidden)keys.clear();});
function walkUpdate(dt){if(!walk)return;let f=0,r=0;if(keys.has('w')||keys.has('arrowup'))f++;if(keys.has('s')||keys.has('arrowdown'))f--;if(keys.has('d')||keys.has('arrowright'))r++;if(keys.has('a')||keys.has('arrowleft'))r--;if(!f&&!r)return;const dir=new THREE.Vector3(-Math.sin(lookYaw)*f+Math.cos(lookYaw)*r,0,-Math.cos(lookYaw)*f-Math.sin(lookYaw)*r).normalize().multiplyScalar(dt*(keys.has('shift')?2.4:1.25));const p=camera.position.clone();p.x+=dir.x;if(!collision(p))camera.position.x=p.x;p.copy(camera.position);p.z+=dir.z;if(!collision(p))camera.position.z=p.z;}
...
try{
 ...
 window.homeApp={scene,model,camera,controls,records,cat,house,smart,terrace,cabinetry,tv,visitor,state:stateStore,go,diagnostics,openNotes:()=>personal.openNotes(),openLibrary:()=>personal.openLibrary(),openMusic:()=>personal.openMusic(),openPortfolio};document.body.dataset.ready='true';
